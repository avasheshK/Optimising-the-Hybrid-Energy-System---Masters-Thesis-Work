...
def revenue_generated():
    ...
    problem += (
        ...
        + pl.lpSum([max(0,solar_production_actual[t] - solar_production_forecast[t]) * positive_imbalance_price[t] + min(0,solar_production_actual[t] - solar_production_forecast[t]) * negative_imbalance_price[t] for t in range(data_points)])
    )
 ...
 # Print the selling decisions
    print("\nSelling decisions:")
    for t in time_frame:
        intraday_val = pl.value(day_ahead_vars[t])
        if intraday_val > 0:
